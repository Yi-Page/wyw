// description: hitomi.la 漫画源 — 搜索/分类/详情/阅读（由 venera hitomi.js v1.1.2 移植，转换脚本自动生成）
// type: manga
//
// 由 venera 源自动转换：
...
async function get_single_galleryblock(gid) {
  ...
  return parseGalleryBlockInfo(res);
}
...
async function get_gallery_detail(gid) {
  ...
  return parseGalleryDetail(resp);
}
...
class hitomi extends PluginSource {
    // ================================================================
    // wyw 适配层（转换脚本统一注入）
    ...
    constructor() {
        ...
        if (typeof this.init === 'function') {
            try {
                Promise.resolve(this.init()).catch((e) => console.warn('[hitomi]', 'init 失败:', e))
            } catch (e) {
                console.warn('[hitomi]', 'init 失败:', e)
            }
        }
    }
    ...
    async getDetail(id) {
        ...
        console.log('[hitomi]', '详情:', title, '章节数:', isGallery ? 0 : chapters.length)
    ...
    async loadChapterImages(comicId, epId) {
        ...
        const urls = raw
            ...
            .filter((u) => !!u)
        if (urls.length === 0) {
            console.error('[hitomi]', '章节图片解析为空: comicId=' + comicId + ' epId=' + epId)
            throw '章节图片解析为空'
        }
        console.log('[hitomi]', '图片数:', urls.length)
        return urls
    }
