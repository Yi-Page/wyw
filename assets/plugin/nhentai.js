// description: nhentai 漫画源 — 搜索/分类/详情/阅读（由 venera nhentai.js v1.1.0 移植，转换脚本自动生成）
// type: manga
//
// 由 venera 源自动转换：
...
class nhentai extends PluginSource {
    // ================================================================
    // wyw 适配层（转换脚本统一注入）
    ...
    constructor() {
        ...
        if (typeof this.init === 'function') {
            try {
                Promise.resolve(this.init()).catch((e) => console.warn('[nhentai]', 'init 失败:', e))
            } catch (e) {
                console.warn('[nhentai]', 'init 失败:', e)
            }
        }
    }
    ...
    async getDetail(id) {
        ...
        console.log('[nhentai]', '详情:', title, '章节数:', isGallery ? 0 : chapters.length)
    ...
    async loadChapterImages(comicId, epId) {
        ...
        const urls = raw
            ...
            .filter((u) => !!u)
        if (urls.length === 0) {
            console.error('[nhentai]', '章节图片解析为空: comicId=' + comicId + ' epId=' + epId)
            throw '章节图片解析为空'
        }
        console.log('[nhentai]', '图片数:', urls.length)
        return urls
    }
    ...
    async loadApiGalleries(url) {
        ...
        return this.parseComicListFromApi(JSON.parse(res));
    }
    ...
    async persistAuthFromCookies() {
        // wyw 无 Network.getCookies API：Cookie 由框架自动同步，这里不再手动提取 token cookie
        return ""
    }
    ...
    _comicApi = {
        ...
        loadInfo: async (id) => {
            ...
            for (let field of document.querySelectorAll("div.tag-container")) {
                let name = ((field.childrenInfo && field.childrenInfo[0] && field.childrenInfo[0].text) || field.text || '').trim().replaceAll(':', '')
                if(name === "Uploaded") {
                    continue;
                }
