...
class WebViewVideoSourceService implements IVideoSourceService {
  ...
  Future<VideoSource> resolve(
    ...
    _resolveTail =
        resolveFuture.then<void>((_) {}, onError: (Object e, StackTrace s) {
      WywLogger().w(
          '${LogTag.webview} 串行队列中视频源解析后台任务失败 episodeUrl=$episodeUrl（不影响主流程）',
          error: e);
    });
    return resolveFuture;
  }

  /// 创建 WebView；`environment_creation_failed`（Android 创建时序竞态，
  /// 常见于前后台切换/快速重进）时重试一次。
  Future<VideoWebviewController> _createWebviewWithRetry() async {
    VideoWebviewController create() =>
        VideoWebviewControllerFactory.getController();
    try {
      final controller = create();
      await controller.init();
      return controller;
    } on PlatformException catch (e) {
      if (e.code != 'environment_creation_failed') rethrow;
      WywLogger().w(
          '${LogTag.webview} WebView 创建失败（environment_creation_failed），重试一次');
      final controller = create();
      await controller.init();
      return controller;
    }
  ...
  Future<VideoSource> _runResolve(
    ...
    if (_webview == null) {
      _webview = await _createWebviewWithRetry();
