...
class TimedShutdownService {
  ...
  void _onTimerExpired() {
    ...
    try {
      ...
      WywLogger().e('${LogTag.player} 定时关闭回调执行失败', error: e, stackTrace: s);
    }
          ...
          actions: [
            TextButton(
              onPressed: () {
                ...
                WywDialog.dismiss();
                repeat();
                WywDialog.showToast(message: '已重新开始 $_lastSetMinutes 分钟定时');
              },
              child: const Text('重复'),
            ),
            TextButton(
  ...
  /// Uses WywDialog to avoid context-related resource leaks
  /// [onExpired] callback is invoked when timer expires (before showing dialog)
...
class _CustomTimerDialogState extends State<_CustomTimerDialog> {
  ...
  void _confirm() {
    ...
    if (totalMinutes <= 0) {
      WywDialog.showToast(message: '请选择有效的时间');
      return;
    }
    WywDialog.dismiss();
    if (widget.autoStart) {
