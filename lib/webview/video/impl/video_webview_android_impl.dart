...
import 'package:wyw/utils/media.dart';
...
class VideoWebviewAndroidImpl
  ...
  Future<void> init() async {
    // 与播放/下载解析等其他 WebView 串行创建，避免并发创建触发
    // environment_creation_failed。
    await serializeWebviewCreation(() async {
      headlessWebView ??= PlatformHeadlessInAppWebView(
        PlatformHeadlessInAppWebViewCreationParams(
          ...
          onWebViewCreated: (controller) {
            WywLogger().i('${LogTag.webview} 视频解析 WebView 已创建');
  ...
  Future<void> loadUrl(String url, {int offset = 0}) async {
    ...
    if (!hasInjectedScripts) {
      addJavaScriptHandlers(false);
      await addUserScripts(false);
