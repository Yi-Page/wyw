...
abstract class _PlayerDebugController with Store {
  ...
  Future<void> setup(
    ...
    playerLogSubscription = player.stream.log.listen((event) {
      ...
      if (playerDebugMode) {
        WywLogger().i('${LogTag.player} MPV 日志 $event', forceLog: true);
      }
    });
    ...
    if (!isCurrentPlayer(player)) return;
    // media_kit on the wyw pinned version may not expose
    // `player.stream.videoBitrate`; subscribe defensively.
    try {
      playerVideoBitrateSubscription =
          player.stream.videoBitrate.listen((event) {
        playerVideoBitrate = event.toString();
      });
    } catch (e) {
      WywLogger().d('${LogTag.player} 订阅 videoBitrate 流失败（已忽略）', error: e);
