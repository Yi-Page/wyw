...
import 'package:wyw/services/storage/playback_download_mode.dart';
...
class _PlayerSettingsPageState extends State<PlayerSettingsPage> {
  ...
  late PlaybackDownloadMode playbackDownloadMode;
  ...
  final MenuController playbackDownloadModeMenuController = MenuController();
  ...
  void _loadSettingsFromStorage() {
    ...
    playbackDownloadMode = PlaybackDownloadMode.fromValue(
      GStorage.getSetting<String>(SettingsKeys.playbackDownloadMode),
    );
  }

  Future<void> resetPlayerSettings() async {
    final bool shouldReset = await WywDialog.show<bool>(
    ...
    WywDialog.showToast(message: '已恢复默认播放设置');
  }
  ...
  Future<int?> _showSkipTimeChangeDialog(
    ...
    return WywDialog.show<int>(builder: (context) {
      String input = '';
      return AlertDialog(
        title: Text(title),
        ...
        actions: <Widget>[
          ...
          TextButton(
            onPressed: () async {
              ...
              if (newValue == null) {
                WywDialog.showToast(message: '请输入数字');
                return;
              }

              if (newValue <= 0) {
                WywDialog.showToast(message: '请输入大于0的数字');
                return;
              }
              // 以新设置的值弹出
              WywDialog.dismiss(popWith: newValue);
            },
            child: const Text('确定'),
          ),
        ],
  ...
  void updatePlaybackDownloadMode(PlaybackDownloadMode mode) {
    GStorage.putSetting<String>(SettingsKeys.playbackDownloadMode, mode.value);
    setState(() {
      playbackDownloadMode = mode;
    });
  }
      ...
      child: SettingsDetailScaffold(
        ...
        body: SettingsList(
          sections: [
              ...
              tiles: [
                ...
                if (Platform.isAndroid) ...[
                  SettingsTile(
                    ...
                    description: const Text('选择视频输出方式'),
                  ),
                ],
                SettingsTile.switchTile(
            ...
            SettingsSection(
              ...
              tiles: [
                SettingsTile.switchTile(
                  ...
                  description: const Text('应用退到后台或熄屏时继续播放音频'),
                ...
                if (Platform.isAndroid)
                  SettingsTile.switchTile(
                    ...
                    title: const Text('自动进入画中画'),
                    description: const Text('切到后台时，自动进入画中画'),
                    initialValue: androidAutoEnterPIP,
                  ),
                SettingsTile.switchTile(
                  leading: Icons.ads_click_rounded,
                  onToggle: (value) async {
                  ...
                  description: const Text('强制启用HLS广告过滤，忽略规则设置'),
                ...
                if (!isDesktop())
                  SettingsTile.switchTile(
                    ...
                    description: const Text('竖向滑动调节音量和亮度'),
            ...
            SettingsSection(
              title: const Text('速度与快进'),
              tiles: [
                SettingsSliderTile(
                  leading: Icons.speed_rounded,
                  title: const Text('默认倍速'),
                  description: const Text('设置播放器的默认播放速度'),
                ...
                SettingsSliderTile(
                  leading: Icons.double_arrow_rounded,
                  title: const Text('默认方向键/长按倍速'),
            ...
            SettingsSection(
              title: const Text('下载协作'),
              tiles: [
                SettingsTile(
                  leading: Icons.download_for_offline_rounded,
                  onPressed: (_) async {
                    if (playbackDownloadModeMenuController.isOpen) {
                      playbackDownloadModeMenuController.close();
                    } else {
                      playbackDownloadModeMenuController.open();
                    }
                  },
                  title: const Text('播放下载模式'),
                  description: const Text(
                    '进入播放前的默认协作方式：无任务时可选 只播放 / 边播放边下载 / '
                    '只下载不进入；有任务时可选 直接播放 / 合流播放 / 不播放',
                  ),
                  value: MenuAnchor(
                    consumeOutsideTap: true,
                    controller: playbackDownloadModeMenuController,
                    builder: (_, __, ___) {
                      return Text(
                        playbackDownloadMode.label,
                      );
                    },
                    menuChildren: [
                      for (final mode in PlaybackDownloadMode.values)
                        MenuItemButton(
                          requestFocusOnHover: false,
                          onPressed: () => updatePlaybackDownloadMode(mode),
                          child: Container(
                            height: 48,
                            ...
                            child: Align(
                              ...
                              child: Text(
                                mode.label,
                                style: TextStyle(
                                  color: mode == playbackDownloadMode
                                      ? Theme.of(context).colorScheme.primary
                                      : null,
                                ),
                              ),
            ...
            SettingsSection(
              ...
              tiles: [
                SettingsTile(
                  ...
                  title: const Text('恢复默认设置'),
                  description: const Text('将播放相关设置恢复为默认值'),
                ),
              ],
            ),
          ],
