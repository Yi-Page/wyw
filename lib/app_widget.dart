...
import 'package:wyw/bean/settings/color_type.dart';
...
import 'package:wyw/utils/theme.dart';
...
class _AppWidgetState extends State<AppWidget>
  ...
  Future<void> _configurePreferredDisplayMode() async {
    ...
    try {
      ...
      WywLogger().e('${LogTag.app} 设置屏幕刷新率失败', error: e);
    }
  }
  ...
  Color _storedThemeColor() {
    ...
    if (defaultThemeColor == 'default') {
      // 默认种子色引用 color_type 的 palette，保持单一来源
      return colorThemeTypes.first['color'] as Color;
    }
  ...
  void _syncWindowsTitleBarBrightness(ThemeProvider themeProvider) {
    ...
    windowManager.setBrightness(brightness).catchError((e) {
      WywLogger().w('${LogTag.app} 设置窗口标题栏亮度失败', error: e);
    });
  }
        ...
        WywDialog.show(onDismiss: () {
          ...
          return AlertDialog(
            ...
            actions: [
              ...
              const TextButton(onPressed: WywDialog.dismiss, child: Text('取消')),
            ],
          );
        });
    }
  ...
  void didChangeAppLifecycleState(AppLifecycleState state) async {
    ...
    if (state == AppLifecycleState.paused) {
      WywLogger().i('${LogTag.app} 应用进入后台 state=paused');
    } else if (state == AppLifecycleState.resumed) {
      WywLogger().i('${LogTag.app} 应用回到前台 state=resumed');
    } else if (state == AppLifecycleState.inactive) {
      WywLogger().i('${LogTag.app} 应用进入非活跃状态 state=inactive');
    }
  }
  ...
  Future<void> didChangePlatformBrightness() async {
    ...
    WywLogger().i('${LogTag.app} 系统亮度变化 themeMode=${themeProvider.themeMode}');
  ...
  Future<void> _handleTray() async {
    if (Platform.isWindows) {
      // Windows 托盘必须用经典 BMP 条目的 .ico（LoadImage 不支持 PNG 压缩条目）
      await trayManager.setIcon('assets/images/logo/logo_tray.ico');
    } else if (Platform.environment.containsKey('FLATPAK_ID') ||
        Platform.environment.containsKey('SNAP')) {
      await trayManager.setIcon('io.github.Yi-Page.wyw');
    } else {
      await trayManager.setIcon('assets/images/logo/logo.png');
    }

    if (!Platform.isLinux) {
      await trayManager.setToolTip('Wyw');
    }

    Menu trayMenu = Menu(items: [
      MenuItem(key: 'show_window', label: '显示窗口'),
      MenuItem.separator(),
      MenuItem(key: 'exit', label: '退出 Wyw')
    ]);
